...
def test_downvote():
    test_id = '22e47f97-11ca-4c3c-8e77-f3068fddaf6e'
    orig_votes = ''
    before_downvote = requests.get("http://44.238.226.5:80/api/v1/leaderboard/", auth=(username, password))
    ...
    for i in op['Items']:
        if i['music_id'] == test_id:
            orig_votes = i['Votes']
            break
    response = requests.post("http://44.238.226.5:80/api/v1/leaderboard/downvote/22e47f97-11ca-4c3c-8e77-f3068fddaf6e",
                             auth=(username, password))

    after_downvote = requests.get("http://44.238.226.5:80/api/v1/leaderboard/", auth=(username, password))
...
def test_tabletopper():
    response = requests.get("http://44.238.226.5:80/api/v1/leaderboard/tabletopper", auth=(username, password))
    assert response.status_code == 200
